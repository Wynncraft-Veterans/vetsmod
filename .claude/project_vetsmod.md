...
- `org.wynnvets.util` — cross-cutting shared singletons, two classes: `HttpClients` (the one `HttpClient` that eighteen network-touching classes hold) and `Json` (the one `Gson` that twenty-one classes read). Both exist in order to be shared state, so both are described where that matters — see [vetsmod_networking.md §3](vetsmod_networking.md)
