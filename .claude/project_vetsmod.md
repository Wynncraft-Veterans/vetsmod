...
- `org.wynnvets.util` — cross-cutting helpers with no home of their own, three classes. Two are shared state and exist in order to be shared: `HttpClients` (the one `HttpClient` that seventeen network-touching classes hold) and `Json` (the one `Gson` that twenty classes read), both described where that matters — see [vetsmod_networking.md §3](vetsmod_networking.md). The third, `ContainerScreens`, is stateless: the two "which container screen is open" predicates, discriminating on container id and on title respectively. Its callers are all in `distribute/`, but the id one also drives the *Manage* menu, so it is not Members-specific — see [vetsmod_distribute.md](vetsmod_distribute.md) §6 point 2. **The package is not a shared-state package**; that was a description of its first two residents, not a charter
