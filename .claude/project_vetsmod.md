...
- `org.wynnvets.queue` — `QueueStateManager`, `QueueDetector`, `QueueStateListener`. Tracks Wynncraft world-queue state from the queue title + world-state signals. Among its readers, `GuildChatDispatcher` routes a Returners member's own `/g` through the WS as `type:"queue"` while queued (the in-game `/g` is observed to be dropped then), and `OutboundDisplayHandler` stops dropping WS-delivered `guild` frames while queued.
