...
2. **`isOutlineSuppressionActive()` / `isAggressiveActive()` are the cheap public flags** for "are we doing the anni-active rendering thing right now". Each is a `volatile boolean` — safe to query from render-thread mixins without extra locking. Their freshness differs: `AnniAggressiveTicker.tick` recomputes unconditionally and fails closed in its catch, while `AnniOutlineTicker.tickInner` writes `suppressionActive` only on the paths that reach the gate evaluation. Expose a parallel flag from any new ticker with its own gate semantics.
