...
All twelve pages are a `buildX` returning the page plus a `helpX` that sends it, so the page is a value a test can compare — see `HelpCommandsTest`, which pins each one as a tree. The pages are assembled from `HelpCommands`' own primitives (`title`, `heading`, `line`, `row`, `requires`, `detailPage`, and the `gray` / `dim` fragment constructors). Every trailing newline belongs to its literal, not to a primitive. `buildHelp` takes its four gates as parameters rather than reading `GuildStateManager`, which is what makes the gate matrix enumerable.

Three colour rules run through every page, and each is a property of the *content* rather than of the call site that wrote it:

- **A row's head splits at its first argument token.** `row` renders the command in YELLOW and everything from the first ` <` or ` [` onward in GOLD, so `/wv debug set <key>` is two runs and `/wv debug trigger charDump` — a literal subcommand, not an argument — is one. `HelpCommands.argumentStart` is the whole rule.
- **`gray` describes and `dim` clarifies.** The GRAY → DARK_GRAY change marks a change of thought, not a line wrap: a description too long for one chat line wraps into a second GRAY fragment rather than spilling into the DARK_GRAY run mid-sentence.
- **The `Requires:` footer colours the *viewer*, not the command.** `HelpCommands.Access` is the four-state ladder — `DENIED` RED, `GRANTED` GREEN, `CONFIRMED` AQUA, `PUBLIC` WHITE — resolved by `access(met, confirmed)`, in which `confirmed` wins outright so a stale local cache cannot report DENIED on a permission the server has granted. GREEN means *this should work*; AQUA means *the server agrees*. Five of the six gated pages take `sessionBacked(met)`, whose confirmation is `isAuthenticatedThisSession()`; `/wv check` is the exception and pairs `isStaff()` with `isConfirmedStaff()`, which is why it can legitimately render GREEN on a command that will still refuse — see the `help-advertises-wv-check-on-a-weaker-gate` gap above. The build halves take an `Access` rather than reading `GuildStateManager`, for the same reason `buildHelp` takes its gates as parameters.
...
- **Snapshot-driven (S2):** when `vetsAnniEnabled` is on (a key slated for retirement: bug `vets-anni-enabled-to-be-retired`) and `AnniSnapshotCache.latest()` is non-null, or the cache is cold and an on-demand `AnniQueryClient.query()` pull brings a snapshot back, renders the rich multi-section view via [AnniCommandRenderer](../src/client/java/org/wynnvets/mwe/anni/render/AnniCommandRenderer.java). Three sub-branches:
  ...
  - *Announced within 2h* — the same body as *Announced 2h+ out*, except that the RSVP line drops its upgrade buttons (`rsvpSection` offers them only more than 2 h out), plus a second `[VETSMOD]` block, `Change Anni Mode? (Click:)`, whose `[Silent] | [Passive] | [Aggressive]` buttons run the `/wv anni <mode>` literals below on click.
- **Legacy fallback:** when `vetsAnniEnabled` is off, or the cache is cold and the on-demand pull resolves with no snapshot, or the renderer declines (an external user with an announced anni), GETs `VetsApi.STAMP` (Unix seconds) and formats the original countdown. Less than 1 hour: red `Annihilation is in X mins!`; 1+ hours: `Annihilation returns in X hours Y mins!`; past (and today also a failed fetch with no future stamp cached, bug `stamp-fallback-says-not-announced-on-cold-network-failure`): `The time for the next annihilation has not yet been announced`. Today a cold-cache pull that hits its 8 s deadline reaches none of this, and `/wv anni` prints nothing (bug `anni-ack-clients-ortimeout-completes-exceptionally`). [vetsmod_mwe_anni.md](vetsmod_mwe_anni.md) §"`/wv anni` render dispatch" owns the full branch list, including the yellow `Annihilation timer is currently unavailable.` state.
...
**Two gates, deliberately different tiers.** Brigadier visibility is `GuildStateManager.isStaffOfAnyGuild()` (server-confirmed staff, or captain+ by Wynntils' live rank, so the command autocompletes reliably for staff); execution is checked at the executor by `DistributeCommands.ensureChief()` → `isChiefOfAnyGuild()`, the only Chief/Owner check in vetsmod for this command. A strategist sees the command and gets a red error on run. That is **not** the defensive-double-check shape of `/wv check` and `/wv invite-force`, which re-check the same predicate their `.requires` uses; here the two gates use different predicates, so the red line is the normal path for staff below chief.
...
The legacy SHA-256 password matching has been removed. Users with stored `vetsWaitlistUnlockTime` / `vetsHonouraryUnlockTime` markers from before the migration get a session-start warning prompting them to run `/vetsmod` in Discord — see [vetsmod_guild_system.md §5](vetsmod_guild_system.md#5-sessionauthwarning). The markers are meant to do nothing else; today they also still unlock client-side (bug `legacy-unlock-markers-still-grant-client-unlock`).
