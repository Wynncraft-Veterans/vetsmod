...
5. **Cold start pulls rather than waiting for the push.** `StampFetcher.fetchStampAndCreateMessage` (the world-join motd path) reads `AnniSnapshotCache.latest()` and on `null` fires a fire-and-forget `AnniQueryClient.query()` before falling through to legacy stamp text. *Note: this intentionally does NOT rely on the push alone. The ~5 min figure is temporary-server's out-of-window `anni_state` cadence (step 2), and vetsmod's own `AnniSnapshotPoller` does not close that gap: it runs on a fixed 30 s schedule but its tick returns immediately unless the cached stamp is inside the 90-minute bar window, floored at `> 0` — and it is the one poller with a non-zero initial delay, so it does not tick at all for the first 30 s (see [vetsmod_networking.md](vetsmod_networking.md) §5). So on cold start a user already inside the anni zone — hours before the window opens — would otherwise see legacy motd and no boss bar until the next push. Today no push arrives (bug `outbound-socket-never-authenticated`), and `AnniSnapshotPoller` has no stamp to poll against while the cache is empty, so the wait would last until some other pull: a reconnect's re-pull, an in-game RSVP, or a cold-cache `/wv anni`.* Any new component that depends on the cache being warm at world-join time gets this for free — but only while `vetsAnniEnabled` is true, since `StampFetcher` gates the pull behind `anniIntegrationActive()`. `AnniWsHandler.register()` installs the reconnect counterpart — a post-connect `AnniQueryClient::query` re-pull.
