...
- Bounces snapshot diffs off the thread that called `AnniSnapshotCache.update` — the one delivering WebSocket frames, or the command thread for a `/wv debug tree anni` injection — via `Minecraft.getInstance().execute(...)`.
