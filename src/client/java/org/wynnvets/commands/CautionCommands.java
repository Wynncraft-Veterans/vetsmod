...
public final class CautionCommands {
  ...
  public static void runCautionGo(String args) {
    ...
    V1ApiManager.sendStaffActionFrame(
        ...
        fields,
        ack -> Minecraft.getInstance().execute(
            () -> renderCommitResult("caution", parsed.target, parsed.rest, ack)));
  }
  ...
  public static void runWarn(String args) {
    ...
    V1ApiManager.sendStaffActionFrame(
        ...
        fields,
        ack -> Minecraft.getInstance().execute(
            () -> renderCommitResult("warning", parsed.target, parsed.rest, ack)));
  }
  ...
  public static void runEject(String args) {
    ...
    V1ApiManager.sendStaffActionFrame(
        ...
        fields,
        ack -> Minecraft.getInstance().execute(
            () -> renderCommitResult("eject", parsed.target, parsed.rest, ack)));
  }
  ...
  private static void renderCautionPreflightOrCommit(
    ...
    if ("would_trigger".equals(status)) {
      ...
      MutableComponent line = Component.literal("⚠ ").withStyle(ChatFormatting.GOLD)
          .append(Component.literal("Adding this caution would trigger a formal ")
              .withStyle(ChatFormatting.YELLOW))
          .append(Component.literal(trigger.toUpperCase())
              .withStyle(ChatFormatting.RED))
          ...
          .append(Component.literal(" (currently " + currentPoints + " pts).")
  ...
  private static void renderCommitResult(
    ...
    boolean blocklisted = ack.has("blocklisted") && !ack.get("blocklisted").isJsonNull()
        && ack.get("blocklisted").getAsBoolean();
    String suggested = ack.has("suggested_dispatch") && !ack.get("suggested_dispatch").isJsonNull()
        ? ack.get("suggested_dispatch").getAsString()
        : "";

    ChatFormatting headerColor = switch (triggered) {
      case "eject"   -> ChatFormatting.RED;
      case "warning" -> ChatFormatting.GOLD;
      default        -> ChatFormatting.YELLOW;
    };
    String headerLabel = switch (triggered) {
      case "eject"   -> "EJECTED";
      case "warning" -> "WARNED";
      default        -> "Cautioned";
    };

    MutableComponent line = Component.literal(headerLabel + " ").withStyle(headerColor)
        .append(Component.literal(resolved).withStyle(ChatFormatting.AQUA));
    if (newTotal >= 0) {
      line.append(Component.literal(" (now " + newTotal + " caution pt"
              + (newTotal == 1 ? "" : "s") + ")").withStyle(ChatFormatting.GRAY));
    }
    if (blocklisted) {
  ...
  private static void renderCautionHistory(String requestedUsername, JsonObject ack) {
    ...
    MutableComponent header = Component.literal("Caution history -- ")
        .withStyle(ChatFormatting.GRAY)
        .append(Component.literal(resolved).withStyle(ChatFormatting.AQUA))
        .append(Component.literal(" -- ").withStyle(ChatFormatting.GRAY))
        .append(Component.literal(total + " pt" + (total == 1 ? "" : "s"))
            .withStyle(totalColor));
    ChatUtils.sendLocalMessage(header);

    JsonArray entries = ack.has("entries") && ack.get("entries").isJsonArray()
    ...
    for (JsonElement el : entries) {
      ...
      ChatFormatting kindColor = switch (kind) {
        case "eject"   -> ChatFormatting.RED;
        case "warning" -> ChatFormatting.GOLD;
      ...
      MutableComponent row = Component.literal("  • ").withStyle(ChatFormatting.GRAY)
          .append(Component.literal(kind + " (+" + points + ")").withStyle(kindColor))
          .append(Component.literal(" by " + actor).withStyle(ChatFormatting.AQUA));
      if (!created.isEmpty()) {
  ...
  private static void dispatchEjectAction(String suggested, String target) {
    ...
    if ("kick".equals(suggested)) {
      ...
      ChatUtils.sendLocalMessage(
          Component.literal("Dispatching /gu kick " + target).withStyle(ChatFormatting.RED));
    } else if ("demote".equals(suggested)) {
      command = "gu rank " + target + " recruit";
      ChatUtils.sendLocalMessage(
          Component.literal("Dispatching /gu rank " + target
