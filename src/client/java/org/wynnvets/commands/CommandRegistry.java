...
public final class CommandRegistry {
                ...
                ClientCommandManager.literal("wv")
                        ...
                        .then(
                                ClientCommandManager.literal("help")
                                        ...
                                        .then(
                                                ClientCommandManager.literal("check")
                                                        .executes(HelpCommands::helpCheck))
                                        .then(
                                                ClientCommandManager.literal("return")
                                        ...
                                        .then(
                                                ClientCommandManager.literal("motd")
                                                        .executes(HelpCommands::helpMotd))
                                        .then(
                                                ClientCommandManager.literal("anni")
                                                        .executes(HelpCommands::helpAnni))
                                        .then(
                                                ClientCommandManager.literal("line")
                        ...
                        .then(
                                ClientCommandManager.literal("invite-force")
                                        .requires(src -> GuildStateManager.isConfirmedStaff())
                                        .then(
                                                ClientCommandManager.argument(
                                                                "playerName",
                        ...
                        .then(
                                ClientCommandManager.literal("anni")
                                        ...
                                        .then(
                                                ClientCommandManager.literal("silent")
                                                        .executes(
                                                                ctx ->
                                                                        anniMode(
                                                                                ctx,
                                                                                AnniMode.SILENT)))
                                        .then(
                                                ClientCommandManager.literal("passive")
                                                        .executes(
                                                                ctx ->
                                                                        anniMode(
                                                                                ctx,
                                                                                AnniMode.PASSIVE)))
                                        .then(
                                                ClientCommandManager.literal("aggressive")
                                                        .executes(
                                                                ctx ->
                                                                        anniMode(
                                                                                ctx,
                                ...
                                ClientCommandManager.literal("config")
                                        ...
                                        .then(
                                                ClientCommandManager.argument(
                                                        ...
                                                        .suggests(
                                                                ConfigCommands.SUGGEST_CONFIG_KEYS)
                        ...
                        .then(
                                ClientCommandManager.literal("line")
                                        .then(
                                                ClientCommandManager.literal("church")
                                                        .executes(ctx -> lineToggle(ctx, "church")))
                                        .then(
                                                ClientCommandManager.literal("scrap")
                                                        .executes(ctx -> lineToggle(ctx, "scrap")))
                                        .then(
                                                ClientCommandManager.literal("bat")
                                                        .executes(ctx -> lineToggle(ctx, "bat")))
                                        .then(
                                                ClientCommandManager.literal("hegea")
                                                        .executes(ctx -> lineToggle(ctx, "hegea")))
                                        .then(
                                                ClientCommandManager.literal("lighthouse")
                                                        .executes(
                                                                ctx ->
                                                                        lineToggle(
                                                                                ctx,
    ...
    private static int check(CommandContext<FabricClientCommandSource> ctx) {
        if (!GuildStateManager.isConfirmedStaff()) {
            ChatUtils.sendLocalMessage(
                    Component.literal(
                                    "You must be confirmed staff (vetsmod authenticated) "
                                            + "to use /wv check.")
    ...
    private static int returnInfo(CommandContext<FabricClientCommandSource> ctx) {
        ReturnFetcher.fetchReturn()
                .thenAccept(returnInfo -> ChatUtils.sendLocalMessageNewBlock(returnInfo));
        return 1;
    }
    ...
    private static int staff(CommandContext<FabricClientCommandSource> ctx) {
        ...
        StaffFetcher.fetchOnlineStaff()
                .thenAccept(staffInfo -> ChatUtils.sendLocalMessageNewBlock(staffInfo));
        return 1;
    }
    ...
    private static int list(CommandContext<FabricClientCommandSource> ctx) {
        ...
        ListFetcher.fetchList()
                .thenAccept(listInfo -> ChatUtils.sendLocalMessageNewBlock(listInfo));
        return 1;
    }
    ...
    private static int anni(CommandContext<FabricClientCommandSource> ctx) {
        StampFetcher.fetchStampAndCreateAnniCommandMessage()
                .thenAccept(
                        blocks -> {
                            if (blocks == null || blocks.isEmpty()) {
                                ChatUtils.sendLocalMessageNewBlock(
                                        Component.literal(
                                                        "Annihilation timer is currently unavailable.")
                                                .withStyle(ChatFormatting.YELLOW));
                                return;
                            }
    ...
    private static int anniMode(CommandContext<FabricClientCommandSource> ctx, AnniMode target) {
