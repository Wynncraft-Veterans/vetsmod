...
public final class StaffFetcher {
    ...
    return HTTP_CLIENT.sendAsync(request, HttpResponse.BodyHandlers.ofString())
        .thenApply(response -> {
          ...
          try {
            ...
            return Component.literal("Error parsing staff data: " + e.getMessage())
                .withStyle(ChatFormatting.RED);
          }
        })
        .exceptionally(e -> Component.literal("Error fetching staff: " + e.getMessage())
            .withStyle(ChatFormatting.RED));
  }
  ...
  private static List<StaffEntry> parseOnlineStaff(String responseBody) {
    ...
    parsed.sort(Comparator
        .comparingInt((StaffEntry entry) -> rankOrder(entry.rank()))
  ...
  private static MutableComponent formatOnlineStaff(List<StaffEntry> onlineStaff) {
    MutableComponent message = Component.literal("Online guild staff (" + onlineStaff.size() + "): ")
    ...
    for (int i = 0; i < onlineStaff.size(); i++) {
      ...
      if (entry.world() != null && !entry.world().isBlank()) {
        message.append(Component.literal(" (" + entry.world() + ")")
