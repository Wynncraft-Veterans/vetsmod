...
public final class PlayerDbProvider implements LookupProvider {
  ...
  public CompletableFuture<ProviderOutcome> lookup(String name) {
    URI uri = URI.create("https://playerdb.co/api/player/minecraft/"
    ...
    return httpClient.sendAsync(req, HttpResponse.BodyHandlers.ofString())
        ...
        .exceptionally(e -> {
          VetsLogger.debug("PlayerDB lookup '{}' threw: {}", name, e.getMessage());
  ...
  private ProviderOutcome translate(HttpResponse<String> resp) {
    ...
    try {
      ...
      boolean success = obj.has("success") && !obj.get("success").isJsonNull()
      ...
      JsonObject player = data != null && data.has("player") && data.get("player").isJsonObject()
          ? data.getAsJsonObject("player") : null;
      if (player == null) return ProviderOutcome.transientMiss();
      String id = player.has("id") && !player.get("id").isJsonNull()
          ? player.get("id").getAsString() : null;
      String username = player.has("username") && !player.get("username").isJsonNull()
          ? player.get("username").getAsString() : null;
